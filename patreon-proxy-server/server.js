...
app.get('/admin/user-info', async (req, res) => {
    try {
        ...
        // Load profile - try Patreon key first, then Discord key
        let profileKey = `${PROFILE_KEY_PREFIX}${userId}`;
...
app.post('/admin/set-achievements', async (req, res) => {
    try {
        ...
        // Find user in all profile types
        const patterns = ['profile:*', 'discord_profile:*', 'user:*'];
        let foundKey = null;
        let foundData = null;
...
app.get('/v3/leaderboard', async (req, res) => {
    try {
        ...
        for (const item of (rankedMembers || [])) {
            const unifiedId = typeof item === 'object' ? (item.member || item.value) : item;
            const xp = typeof item === 'object' ? (item.score) : 0;
    ...
    try {
        ...
        if (!hadLevelReset && !pendingSkillsReset) {
            ...
            if (typeof skill_points === 'number') {
                ...
                const serverSkillCount = (user.unlocked_skills || []).length;
                if (clientSkillCount >= serverSkillCount) {
                    // Client has same or more skills — trust their point total
                    // (covers both: earned new points via level up, or spent points on skills)
...
const DEFAULT_MARQUEE = 'Thanks to Desiree for the avatar/personalities idea! She\'s Such a Good Girl - v5.3.1 is out! ❤️🔒';
...
app.post('/admin/merge-accounts', async (req, res) => {
    try {
        ...
        if (source.discord_id && !target.discord_id) {
            ...
            await redis.set(`discord_index:${source.discord_id}`, target_id);
            ...
            await redis.del(`discord_index:${source.discord_id}`);
        ...
        if (source.patreon_id && !target.patreon_id) {
            ...
            await redis.set(`patreon_index:${source.patreon_id}`, target_id);
            changes.push(`patreon_id transferred: ${source.patreon_id}`);
        } else if (source.patreon_id) {
            await redis.del(`patreon_index:${source.patreon_id}`);
            deletedKeys.push(`patreon_index:${source.patreon_id}`);
        }

        if (source.email && !target.email) {
            target.email = source.email;
            await redis.set(`email_index:${source.email.toLowerCase()}`, target_id);
            changes.push(`email transferred: ${source.email}`);
        } else if (source.email) {
            await redis.del(`email_index:${source.email.toLowerCase()}`);
        ...
        if (override_display_name !== undefined) {
            ...
            await redis.set(`display_name_index:${override_display_name.toLowerCase()}`, target_id);
        ...
        await redis.set(`user:${target_id}`, JSON.stringify(target));
        ...
        await redis.zadd(`leaderboard:${season}`, { score: leaderboardXp, member: target_id });
        changes.push(`leaderboard:${season} updated for target (xp: ${leaderboardXp})`);

        if (source.current_season) {
            await redis.zrem(`leaderboard:${source.current_season}`, source_id);
        ...
        if (source.display_name) {
            ...
            if (currentMapping === source_id) {
                await redis.del(sourceNameKey);
                deletedKeys.push(sourceNameKey);
            }
        }

        // Delete source user record
        await redis.del(`user:${source_id}`);
        ...
        if (source.patreon_id) {
            ...
            if (await redis.get(sourceProfileKey)) {
                await redis.del(sourceProfileKey);
        ...
        if (source.discord_id) {
            ...
            if (await redis.get(sourceDiscordKey)) {
                await redis.del(sourceDiscordKey);
        ...
        if (target.patreon_id) {
            ...
            if (existing) {
                ...
                await redis.set(targetProfileKey, JSON.stringify(profile));
        ...
        if (target.discord_id) {
            ...
            if (existing) {
                ...
                profile.stats = target.stats;
                profile.updated_at = target.updated_at;
                await redis.set(targetDiscordKey, JSON.stringify(profile));
                changes.push(`updated legacy key ${targetDiscordKey}`);
            }
        }

        console.log(`[ADMIN] Merged accounts: ${source_id} -> ${target_id} | Changes: ${changes.join(', ')}`);
