 ...
 * Legacy users (no auth_token_hash) are allowed through with a warning.
 */
function validateAuthToken(req, user) {
    const token = req.headers['x-auth-token'];
    if (!user.auth_token_hash) {
        // Legacy user — no token issued yet. Allow but flag.
...
app.post('/v2/auth/restore-session', async (req, res) => {
    try {
        ...
        user.auth_token_hash = restoreAuthTokenHash;
...
app.get('/v3/leaderboard', async (req, res) => {
    try {
        ...
        if (cached && (Date.now() - cached.time) < LEADERBOARD_V3_CACHE_TTL) {
            ...
            const onlineCount = sliced.filter(e => e.is_online).length;
            return res.json({
                season,
                entries: sliced,
